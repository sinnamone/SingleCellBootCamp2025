...
if (!is.null(ego) && nrow(ego) > 0) {
  ...
  write.csv(ego_df, file = file.path(outdir, paste0("cluster_0", "_GO.csv")), row.names = FALSE)
}

# Optional: MSigDB enrichment
emsig <- enricher(gene =  genes_entrez$ENTREZID,
                  TERM2GENE =  msigdb_C2 %>% dplyr::select(gs_name, entrez_gene) ,
                  universe = background_entrez$ENTREZID)

if (!is.null(emsig) && nrow(emsig) > 0) {
  emsig_df <- as.data.frame(emsig)
  write.csv(emsig_df, file = file.path(outdir, paste0("cluster_", "0", "_MSigDB_Hallmark.csv")), row.names = FALSE)
}
